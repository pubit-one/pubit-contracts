...
interface ILicenseRegistry {
     ...
     */
    event licenseContractAdded(
        bytes32 licenseType,
        address contractAddress,
        bytes4[] actions
    );

    event licenseCallDataUpdated(
        bytes32 license,
        bytes32 action,
        bytes4 oldAction,
        bytes4 newAction
    );
    event actionDataUpdated(
        bytes32[] _actions,
        LicenseLib.ActionCat actionType,
        LicenseLib.ActionReturn actionReturn
    );
    event actionDataAdded(
        bytes32 action,
        LicenseLib.ActionCat actionType,
        LicenseLib.ActionReturn actionReturn
    );
     ...
     */
    function addLincenseType(bytes32 _name, bytes32 _contentType) external;

    /**
     * @dev external function to add content type
     * @param _contentType name of content type
     */
    function addContentType(bytes32 _contentType) external;
}
