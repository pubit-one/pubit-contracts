...
contract LicenseRegistry is ILicenseRegistry {
    ...
    constructor(address _accessRestrictionAddress) {
        ...
        //     revert LicenseLib.InvalidAccessRestriction(
        //         _accessRestrictionAddress
        //     );
        // }
    ...
    function addLicenseContract(
        ...
        bytes32 id = keccak256(
            abi.encode(_contentType, _licenseType, _templateId)
        );
        ...
        for (uint i = 0; i < _actions.length; i++) {
            if (
                actionUsages[_contractAddress][_actions[i]] != bytes32(0) &&
                _actions[i] != bytes4(0)
            ) {
                revert LicenseLib.ActionUsedBefore(
                    _contractAddress,
                    _actions[i]
                );
            }
    ...
    function addLincenseType(
        ...
        licenseTypes[id] = LicenseLib.LicenseMetadata(
            _contentType,
            _licenseType,
            address(0),
            v
    ...
    function addContentType(bytes32 _contentType) external onlyAdmin {
        if (contentTypes[bytes32(_contentType)]) {
    ...
    function getLicenseContracts(
        bytes32 _license
    )
        external
        view
        returns (LicenseLib.LicenseMetadata memory, bytes4[] memory)
    {
        bytes4[] memory _actions = new bytes4[](actions.length);
        for (uint i = 0; i < actions.length; i) {
            _actions[i] = licenseActions[
                licenseTypes[_license].contractAddress
            ][actions[i]];
        }
    ...
    function addActionData(
        bytes32 _action,
        LicenseLib.ActionCat _actionType,
        LicenseLib.ActionReturn _actionReturn
    ) external onlyAdmin {
        ...
        actions = _actions;
        actionData[_action] = LicenseLib.ActionData(
            ind,
            _actionType,
            _actionReturn,
            true,
    ...
    function _init_actionData() internal {
        ...
        actionData[_ac[0]] = LicenseLib.ActionData(
            0,
            LicenseLib.ActionCat.Default,
            LicenseLib.ActionReturn.Data,
            true,
            0,
            0,
            0
        ...
        for (uint128 i = 1; i < _ac.length - 1; i++) {
            actions.push(_ac[i]);
            if (i < 8) {
                actionData[_ac[i]] = LicenseLib.ActionData(
                    i,
                    LicenseLib.ActionCat.PreActions,
                    LicenseLib.ActionReturn.Bool,
                    true,
                    0,
                    0,
                    0
                ...
                actionData[_ac[i]] = LicenseLib.ActionData(
                    i,
                    LicenseLib.ActionCat.GenActions,
                    LicenseLib.ActionReturn.Index,
                    true,
                    0,
                    0,
                    0
                ...
                actionData[_ac[i]] = LicenseLib.ActionData(
                    i,
                    LicenseLib.ActionCat.PostActions,
                    LicenseLib.ActionReturn.Status,
                    true,
                    0,
                    0,
                    0
                ...
                actionData[_ac[i]] = LicenseLib.ActionData(
                    i,
                    LicenseLib.ActionCat.ViewActions,
                    LicenseLib.ActionReturn.Data,
                    true,
                    0,
                    0,
        ...
        // actions = _actions;
        // console.log(actions[17],"actions");
